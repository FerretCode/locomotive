...
func ReconstructLogLine(log *graphql.EnvironmentLog) (logLine json.RawMessage, err error) {
	...
	if err != nil {
		return json.RawMessage{}, fmt.Errorf("failed to append message attribute to object: %w", err)
	}
	...
	if len(log.Attributes) > 0 {
		for _, attr := range log.Attributes {
			...
			if err != nil {
				return json.RawMessage{}, fmt.Errorf("failed to append json attribute to object: %w", err)
			}
		}
	} else {
		jsonObject, err = jsonparser.Set(jsonObject, log.TimestampRaw, "time")
		if err != nil {
			return json.RawMessage{}, fmt.Errorf("failed to append time attribute to object: %w", err)
		}

		jsonObject, err = jsonparser.Set(jsonObject, log.SeverityRaw, "severity")
		if err != nil {
			return json.RawMessage{}, fmt.Errorf("failed to append severity attribute to object: %w", err)
		}
	}
