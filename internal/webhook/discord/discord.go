...
import (
	...
	"github.com/ferretcode/locomotive/internal/logline/reconstructor/reconstruct_discord"
...
func SendWebhookForDeployLogs(logs []environment_logs.EnvironmentLogWithMetadata, cfg *config.Config, client *http.Client) error {
	payload, err := reconstruct_discord.EnvironmentLogLines(logs, cfg.DiscordPrettyJson)
	if err != nil {
